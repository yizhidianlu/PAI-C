# 端到端工作流

> **Reference** · 每个 `/paic-*` 的调用方式、产出位置、典型耗时、跨会话恢复。

<p align="center">
  <img src="paic-workflow.png" alt="PAI-C workflow" width="900">
</p>

> Slash command 使用连字符：`/paic-search` ✓ ；`/paic:search` ✗ 。

首次使用先完成 [getting-started.md](getting-started.md)。本文假定 `paic doctor` 全绿。

> 本文描述常规 9 步主链。Paper-quality 高级阶段（paper_plan / claims / paragraph compose / clusters / revisions / quality gate）见 [paper-plan.md](paper-plan.md) 与 [quality-gate.md](quality-gate.md)，并在文末「Paper-quality 高级阶段」节有简表。

---

## 1. `/paic-init` — 建项目

```text
/paic-init 方向：long-context attention 在医学影像报告生成；目标 NeurIPS 2027。
```

- **产出**：`.paic/project.yaml`（含 title / venue / deadline / related_topics）
- **耗时**：< 1 秒（纯本地，不调 LLM）

> Skill 要求当前目录为空或仅含 `.paic/`，避免在已有项目误初始化。

---

## 2. `/paic-search <query>` — 多源检索

```text
/paic-search 近两年 long-context transformer 在长文本摘要 / 医学报告生成的代表性工作。
```

- **来源**：arXiv（经 arxiv MCP）+ Semantic Scholar；opt-in 多平台见 [external-search.md](external-search.md)
- **去重**：DOI / arxiv_id 主键 + 标题 fuzzy（threshold 0.92）
- **召回校验**：dedupe 之后从 topic 抽 2–4 个英文 core terms，把召回池按标题命中分两段渲染（「强相关·标题命中核心词」/「其他召回」），避免长召回池中明确相关的论文被注意力筛掉
- **产出**：返回候选表（不落盘；`/paic-ingest` 才入库）
- **耗时**：5–15 秒
- **缓存**：S2 响应缓存到 `~/.paic/cache/s2/`，永不过期

### 检索词变体

输入为 **broad topic**（1–3 词、纯概念词、含中文）时，Skill 先生成 5–6 个英文检索词变体供选择：

```text
基于「diffusion video」候选 6 个检索词变体（默认 #1）：
  1. diffusion model video generation       (直接)
  2. latent diffusion video synthesis       (技术)
  3. text-to-video diffusion                  (应用)
  4. video diffusion temporal consistency   (痛点)
  5. efficient video diffusion 2024          (近期)
  6. controllable video generation diffusion (邻近)
请选择：回车 = #1；多选 "1,3"；自定义 "custom: foo bar"
```

回车走默认 #1。多选最多 3 个，超过会拖慢 fan-out（3 query × 5 platform × pacing ≈ 22s）。

输入为 **specific query**（4+ 词且含具体方法 / 数据集 / 年份）时，Skill 跳过 variant 步骤，直接作主 query。

> 未设置 S2 key 时自动降到 0.33 req/s 匿名档，多源搜索仍可执行。

---

## 3. `/paic-ingest <ids>` — 入库 + 下载（+ 可选翻译 / Zotero）

```text
ingest 第 1, 3, 7 篇。           # 默认：只下载、不翻译
ingest 第 1, 3, 7 篇 帮我翻译     # 加任一关键词「翻译/译/中文/translate」→ 触发 opt-in 中文翻译
```

- **元数据**：写入 `.paic/library/selected.yaml`
- **本地存档**：`.paic/library/pdfs/<NNN>_<title_slug>.<ext>`（如 `001_attention_is_all_you_need.md`）——3 位库内累积序号 + 标题 slug（最多 150 字符）。文件名按人类可读优化，与 BibTeX `\cite{KEY}` 解耦
- **平台路由**：
  - **arXiv**：同时下 markdown（供 summarize / draft）与原始 PDF（供阅读）
  - **多平台**（PubMed / bioRxiv / OpenAlex / Crossref / IEEE / ACM 等）：opt-in，需启用外部检索，详见 [external-search.md](external-search.md)
  - **不可下载**（s2 / google_scholar / ssrn / iacr）：仅入库元数据；摘要标注「未下载 N 篇」
- **去重**：按 arxiv_id / DOI 与 `selected.yaml` 校验，重复条目计入 `skipped_duplicates`
- **DOI 黑名单**：订阅墙发布商（IOP / IEEE / Elsevier / Wiley / Springer 等）跳过 OA fallback，避免 europepmc silent 返回错论文 PDF
- **内容校验**：OA fallback 下来的 PDF 走 page-1 三信号校验（作者姓氏 + 标题词命中 + 字符串相似度），不命中**询问用户**后才删
- **节流**：arxiv 6s pace、各平台 per-platform pace；429 自动加长 pace 重试，连续 3 次让用户决定 skip
- **opt-in 中文翻译**：原话含「翻译 / 译 / 中文 / translate」时触发；主对话**不阻塞**，整批委派给后台 subagent 串行翻译（耗时不计主流程），完成时自动通知。产物 `<NNN>_<slug>_zh.pdf`，**仅 arxiv**，需独立装 arxiv-translator skill 到 `~/.claude/skills/`
- **opt-in Zotero 同步**：检测到 zotero-mcp 注册时询问；写入 collection `paic-ingest-<YYYYMMDD>`，详见 [zotero-sync.md](zotero-sync.md)
- **耗时**：每篇 5–30 秒；17 篇 arXiv ≈ 100s。翻译后台跑、不计主流程

---

## 4. `/paic-summarize [id|all]` — 结构化摘要

```text
/paic-summarize all
```

- **读取**：4 级 fallback——arxiv 上游 markdown / 项目本地 markdown / 项目本地 PDF（pypdf 提取，命中后永久缓存）/ 用户显式传入文本
- **平台覆盖**：arXiv 走 markdown 路径；其它平台走 PDF 提取
- **写入**：`.paic/library/summaries/<paper_id>.md`
- **结构**：problem / method / key_results / limitations / techniques / relevance_to_project
- **耗时**：每篇 30–60 秒；首次 PDF 提取额外 ~0.5s（之后缓存命中）
- **PDF 失败恢复**：`encrypted` → `qpdf --decrypt`；`empty_extraction`（扫描版）→ `ocrmypdf`；`corrupt` → 重 `/paic-ingest`。详见 [troubleshooting.md](troubleshooting.md)
- **Host 模式**：当订阅复用启用时由主对话生成、零外部 LLM 调用，详见 [configuration.md](configuration.md#host-orchestration)

---

## 5. `/paic-ideate [focus]` — 生成 idea（含用户筛选）

```text
/paic-ideate focus 在 "如何不堆 KV cache 扩到 200k token"，给出 8 个 idea。
```

工作机制：

1. 后台拉取所有 `summaries/*.md` 作为 grounding，产生 N 张候选 IdeaCard
2. **暂停**等待用户筛选——Claude 列出预览卡询问保留哪几条
3. 用户回复「保留 1, 4, 7」，4-persona 评分（methodology / novelty / impact / reviewer2）后写入 `.paic/ideas/<idea_id>.yaml`

- **跨会话恢复**：暂停在用户筛选这步，关闭 Claude Code 后下次 `/paic-resume` 续跑
- **耗时**：脑暴 1–3 分钟；筛选后 finalize ~10 秒

> IdeaCard 含 `feasibility / novelty / impact` 三维评分 + `composite_score`。

---

## 6. `/paic-experiment <idea_id>` — 实验方案

```text
/paic-experiment 选评分最高的 idea，约束：单张 A6000、3 周内完成。
```

- **写入**：`.paic/experiments/<exp_id>.yaml`
- **结构**：research_questions / hypotheses / datasets / baselines / proposed_method（含伪代码）/ metrics / ablations / compute_budget / success_criteria / threats_to_validity / timeline_weeks
- **耗时**：1–2 分钟，无 interrupt

---

## 7. `/paic-review <experiment_id>` — 4-persona 多轮评审

```text
/paic-review 跑 2 轮 4-persona 评审。
```

最长的一步——4 persona × N 轮 + moderator + verdict。

| 角色 | 职责 |
|---|---|
| methodology | 方法论严谨性 |
| statistics | 统计 + 数据 + reproducibility |
| domain | 领域 fit 与文献定位 |
| reviewer2 | 创新性 + 红队视角 |
| moderator | 综合 4 个 critique 出 issue list |
| verdict | 终判 accept/major/minor/reject |

工作机制：

1. 拉取 5–10 篇相关 summary 进入上下文
2. 每轮：4 persona 串行 critique → moderator 综合 → **暂停等用户 rebuttal**
3. 用户提交 rebuttal（「针对 issue #1 加入 LongBench baseline；issue #2 同意将 seed 提到 5」）
4. PAI-C 更新 experiment / rebuttals 并决定是否进入下一轮
5. 跑完 `max_rounds` 或 issue 全部 resolved → 出 verdict

- **产出**：`.paic/reviews/<exp_id>/{transcript.yaml, round_1.md, ..., verdict.yaml}`
- **耗时**：每轮 4–8 分钟。状态自动落盘，关闭 Claude Code 不影响恢复

> 不同 persona 可路由到不同 LLM（reviewer2 用 Opus、其余用便宜模型），见 [configuration.md](configuration.md#路由-routing)。

---

## 8. `/paic-draft <stage>` — LaTeX 三阶写作

### 8.1 fill — 模板填充（v0.1）

```text
/paic-draft fill 用 NeurIPS 模板，基于刚 verdict 的 idea + experiment 起骨架。
```

- **模板**：内置 `cvpr` / `neurips` / `ieee`；自定义模板放 `<project>/.paic/templates/<name>/`（自动识别，名字大小写不敏感）。详见 [custom-templates.md](custom-templates.md)
- **写入**：`.paic/drafts/main.tex` + `sections/0X_*.tex` + `refs.bib` + 模板自带 `.sty / .cls / .bst / figures` 自动拷贝
- **填充范围**：title / abstract / introduction（contributions 列表）/ method 框架 / experiment setup / 占位 results 段
- **scaffold 新 venue**：找不到模板时 SKILL 提议从 cvpr / neurips / ieee 之一派生骨架，之后用户改 `\usepackage{...}` + 拖入 venue 官方 `.sty`
- **不做**：不跑编译、不写完整段落（polish + compose 负责）
- **耗时**：5–15 秒（纯模板渲染，不调 LLM）

### 8.2 polish — 段落级 LLM 重写（v0.2）

```text
/paic-draft polish 01_intro --mode tighten
/paic-draft polish 03_method --mode expand     # 把 fill 留下的 TODO 占位扩写完整
/paic-draft polish 02_related --mode formalize --instruction "用第三人称"
```

- **5 模式**：`tighten` / `clarify` / `formalize` / `expand` / `proofread`；可附自由指令
- **写盘 + 备份**：覆盖原文件，旁置 `<section>.tex.bak.<UTC>`
- **结构 guard**：保留所有 `\cite{}` 键、不引入新 cite key、`\begin/\end` 配对、大括号配对——任一未通过则不写盘并报错
- **expand 模式**：扩写 fill 留下的 TODO 占位，需要绑定 `idea_id`（Skill 可从最新 idea 推断）
- **dry-run**：`--dry-run` 仅输出 diff，不写盘
- **支持 host 模式**（订阅复用）

### 8.3 compose — 整段生成 + 引用对齐（v0.3）

```text
/paic-draft compose 02_related --mode from_stub
/paic-draft compose 01_intro --mode from_scratch --target-words 800
```

- **2 模式**：`from_stub`（默认，把当前文件作为 outline 扩写）、`from_scratch`（从 idea + library 重新生成，忽略当前文件）
- **引用对齐**：把 library（cap 40 篇）的 `[cite_key] title (author, year) — 一句概括` 注入 prompt，LLM 仅从该 whitelist 选 cite。任何 `\cite{KEY}` 不在库内则不写盘并报错
- **章节默认字数**：intro 800 / related 700 / method 900 / experiments 800 / conclusion 220 / abstract 200（`--target-words` 覆盖）
- **写盘 + 备份**：与 polish 同
- **错误恢复**：报错列出 `cite_keys_missing_from_library`——ingest 缺失论文后重试，或换 mode
- **支持 host 模式**（订阅复用）

**典型流程**：fill → compose（写出含真实引用的段落）→ polish（细调措辞）。

`compose` 还有第 3 模式 `paragraph`（outline → write → polish 三步），适合长 section（intro / related / experiments），需 `paper_plan + claims` 齐全。详见 [paper-plan.md § 4](paper-plan.md)。

---

## 9. `/paic-figure <stage>` — 论文配图（raster）

启用前置：`providers.images.enabled: true`，详见 [configuration.md → images](configuration.md#images-paic-figure)。

### 9.1 plan — 提议图位

```text
/paic-figure plan
```

- **读取**：当前 polished `.tex`；否则回退到 idea + experiment
- **产出**：`.paic/figures/_plan.yaml`，含 ≤4 个 slot（`teaser` / `concept` / `domain`），每条带 `section_hint` / `position_hint` / `scene_description` / `caption_hint` / `rationale`
- **耗时**：10–20 秒（不调图像 API）

### 9.2 generate — 渲染图片

```text
/paic-figure generate teaser
```

- **流程**：合成 academic-style 图像 prompt → 调用图像 API → 写入 `.paic/figures/<slot>/v1.png` + `meta.yaml`
- **返回**：图片路径 + 即用的 `\begin{figure}...\includegraphics{figures/<slot>/v1.png}...\end{figure}` 片段
- **耗时**：10–30 秒/张（视后端）；gpt-image-1 在 `quality=high` 下约 ¥0.5–2/张（中转站）

### 9.3 edit / variant — 改图

```text
/paic-figure edit teaser "调暗整体色调，增加海洋元素"
/paic-figure variant teaser n=2
```

- `edit` 基于最新版微调，写入 `v<n>_edit.png`，meta 记录 `parent_version`
- `variant` 基于最新版生成 N 个备选，写入 `v<n>_variant.png`
- gpt-image-1 无 native variations，`variant` 走 `edit` + "alternative variation" 提示模拟
- 多次 edit 会偏移；建议每 slot 至多 edit 2 次，不满意则修改 `scene_description` 重新 generate

### 范围限制

- **不适合**：架构图（boxes + arrows + 标签）、定量结果图（曲线 / 柱状图）、含精确文字的图表
- **替代方案**：架构图用 TikZ；结果图用 matplotlib + 真实实验数据

---

## 跨会话恢复

下次启动 Claude Code，第一句：

```text
/paic-resume
```

列出 paused / awaiting_input 的 run。继续指定 run：

```text
/paic-resume 01HX8K... rebuttal："针对 issue #1，..."
```

PAI-C 自动从 `.paic/state/` 下的检查点续跑。

---

## 状态查看

```text
/paic-status
```

输出：项目元信息、library / ideas / experiments / reviews 计数、当前活跃的后台 run。

---

## Paper-quality 高级阶段

主链 9 步之外，PAI-C 提供一组 opt-in **paper-quality** 工具把「散点式生成 → 一致性论文」串起来。完整教程见 [paper-plan.md](paper-plan.md) 与 [quality-gate.md](quality-gate.md)；以下是简表。

| 命令 / 功能 | 作用 | 文档 |
|---|---|---|
| `/paic-paper-plan` | 生成 / 更新 `paper_plan.yaml`：thesis、contributions、section_plan、terminology、symbols | [paper-plan.md § 1](paper-plan.md) |
| Claim ledger | 强声明留痕（compose / polish 自动抽取；validate 跨 claim 检查支持） | [paper-plan.md § 2](paper-plan.md) |
| Section-aware retrieval | BM25 + MMR 按 section 检索；library > 40 篇时 compose 自动启用 | [paper-plan.md § 3](paper-plan.md) |
| `/paic-draft compose --mode paragraph` | outline → write → polish 三步 pipeline，长 section 一致性更好 | [paper-plan.md § 4](paper-plan.md) |
| Related-work clustering | library 分 3-5 群；`compose 02_related` 自动一群一段 + contrast | [paper-plan.md § 5](paper-plan.md) |
| Revision queue | review 评论转 RevisionTask，跨轮跟踪 | [paper-plan.md § 6](paper-plan.md) |
| Claim-bound figure plan | 每个 contribution 必须有图 / 表 / 算法绑定或 `no_visual_reason` | [paper-plan.md § 5](paper-plan.md) |
| `/paic-finalize` | 提交前 8 类 paper-level 检查 + overrides | [quality-gate.md](quality-gate.md) |

**典型嵌入位置**：`/paic-experiment` 之后跑 `/paic-paper-plan`；`/paic-review` 后让评论自动转 RevisionTask；compose `02_related` 之前先聚类；提交前 `/paic-finalize`。

---

## 端到端串接

```text
方向：transformer 在脑电信号自动诊断。请按 init → search → ingest → summarize
→ ideate → experiment → paper-plan → review → draft fill → compose paragraph
→ figure plan → finalize 串起来，每个用户 checkpoint 暂停等我。
```

Claude 自行按顺序串接 skill。

---

## 何时进入 Paper-quality 阶段？

`/paic-paper-plan` / `/paic-finalize` 是写作 / 提交期工具，**不是**所有项目都该走。判据：

| 你的状态 | 推荐路径 |
|---|---|
| 还在探索期，不确定核心论点 | 继续 `/paic-ideate` 多轮 + `/paic-experiment`；先别起 paper plan |
| 已锁定 1-2 个核心 idea，要开始写第一版 | `/paic-paper-plan` 锁全局论点 + 章节意图，再 `/paic-draft fill` |
| 草稿已成形，要做投稿前检查 | `/paic-finalize` 跑 8 类 paper-level 一致性 |

详细 paper-quality 工具链（claim ledger / paragraph compose / cluster / revision queue）见 [paper-plan.md](paper-plan.md)；提交前检查见 [quality-gate.md](quality-gate.md)。
